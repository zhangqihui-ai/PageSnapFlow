#!/usr/bin/env python3
"""TapTap Tap制造 (b01_t03): scroll down + screenshot until bottom."""
...
def page_has_content(adb: AdbDevice, *, top_ratio: float = 0.18, bottom_ratio: float = 0.90, min_std: float = 18.0) -> bool:
    temp = Path(tempfile.gettempdir()) / f"tapmake_probe_{time.time_ns()}.png"
    try:
        adb.screencap(temp)
...
def wait_for_page_content(adb: AdbDevice, timeout_ms: int = 12000) -> bool:
    ...
    while time.time() < deadline:
        if page_has_content(adb):
            print("Tap制造 page content detected.", flush=True)
            adb.sleep_ms(700)
            return True
        adb.sleep_ms(400)
    print("Warning: Tap制造 page content not detected before timeout.", flush=True)
    return False


def navigate_to_tap_make(adb: AdbDevice, config: dict) -> None:
    bottom = None
    top = None
    ...
    if not bottom or not top:
        raise RuntimeError("Could not find b01/t03 in taptap_tabs.yaml")

    adb.tap_point(bottom["point"])
    adb.sleep_ms(300)
    adb.tap_point(top["point"])
    adb.sleep_ms(500)
    print("Navigated: find_games -> tap_make", flush=True)
    wait_for_page_content(adb)


def load_tapmake_capture_cfg(adb_config: dict) -> dict:
    return dict(adb_config.get("tapmake_capture", {}))
...
def resolve_output_dir(args: argparse.Namespace) -> Path:
    ...
    if args.until_bottom:
        label = "tapmake_bottom"
    elif args.full:
        label = "tapmake_full"
    else:
        label = f"tapmake_{args.shots}"
    return _ROOT / "screenshots" / "taptap_lite" / f"pilot_{label}_{stamp}"


def main() -> int:
    parser = argparse.ArgumentParser(description="TapTap Tap制造 scroll capture.")
    ...
    parser.add_argument(
        "--until-bottom",
        action="store_true",
        help="Keep scrolling until 15 consecutive dup frames or max_scroll_swipes (500)",
    )
    parser.add_argument("--prefix", default="b01_t03", help="Filename prefix")
    parser.add_argument("--device", default=None)
    parser.add_argument(
        "--skip-nav",
        action="store_true",
        help="Already on Tap制造 tab (skip navigation and long splash wait)",
    )
    parser.add_argument(
        "--resume-dir",
        default=None,
    ...
    adb_cfg_path = _ROOT / "config" / "taptap_adb.yaml"
    adb_cfg = yaml.safe_load(adb_cfg_path.read_text(encoding="utf-8"))
    tapmake_cfg = load_tapmake_capture_cfg(adb_cfg)
    ...
    if args.until_bottom or args.full or args.resume_dir:
        capture_overrides["scroll_stop_after_dupes"] = int(
            tapmake_cfg.get("scroll_stop_after_dupes", 15 if args.until_bottom else 3)
        )
        capture_overrides["duplicate_threshold"] = float(
            tapmake_cfg.get("duplicate_threshold", timing.get("duplicate_threshold", 0.95))
        )

    swipe_batch = int(tapmake_cfg.get("max_scroll_swipes", 500))
    if args.until_bottom:
        dup_threshold = float(capture_overrides.get("duplicate_threshold", 0.992))
        skip_dupes = True
        if resume:
            max_swipes = resume["scroll_n"] + swipe_batch
        else:
    ...
    if args.skip_nav or args.resume_dir:
        ...
        navigate_to_tap_make(adb, tabs)

    if args.until_bottom:
        mode = (
            f"until bottom (scroll {start_scroll_index}..{max_swipes}, "
            f"stop after {capture_overrides.get('scroll_stop_after_dupes', 15)} dupes)"
        )
    elif args.full:
        mode = f"full scroll (max {max_swipes} swipes)"
    else:
        mode = f"{args.shots} screenshot(s)"
    print(f"Capturing Tap制造 {mode}...", flush=True)
    ...
    marker = _ROOT / "screenshots" / "taptap_lite" / "latest_tapmake_output_dir.txt"
