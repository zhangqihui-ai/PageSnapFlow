#!/usr/bin/env python3
"""TapTap 排行榜-热玩榜 (b02_t03): scroll to top, then scroll + screenshot until bottom."""
...
PREFIX = "b02_t03"
FOLDER_TAG = "热玩榜"
MARKER_NAME = "latest_rank_playing_output_dir.txt"
...
def page_has_content(adb: AdbDevice, *, top_ratio: float = 0.20, bottom_ratio: float = 0.90, min_std: float = 18.0) -> bool:
    temp = Path(tempfile.gettempdir()) / f"rank_playing_probe_{time.time_ns()}.png"
    try:
        adb.screencap(temp)
...
def navigate_to_playing_rank(adb: AdbDevice, config: dict) -> None:
    bottom = None
    top = None
    ...
    adb.sleep_ms(500)
    print("Navigated: ranking -> playing_rank", flush=True)
...
def load_scroll_capture_cfg(adb_config: dict) -> dict:
    return dict(adb_config.get("rank_playing_capture") or adb_config.get("ranking_list_capture") or {})
...
def main() -> int:
    ...
    if args.skip_nav or args.resume_dir:
        ...
        navigate_to_playing_rank(adb, tabs)
