#!/usr/bin/env python3
"""TapTap 安利墙 (b01_t06): scroll down + screenshot until bottom."""
...
PREFIX = "b01_t06"
FOLDER_TAG = "安利墙"
MARKER_NAME = "latest_sharewall_output_dir.txt"
...
def page_has_content(adb: AdbDevice, *, top_ratio: float = 0.20, bottom_ratio: float = 0.90, min_std: float = 18.0) -> bool:
    temp = Path(tempfile.gettempdir()) / f"sharewall_probe_{time.time_ns()}.png"
    try:
        adb.screencap(temp)
...
def navigate_to_share_wall(adb: AdbDevice, config: dict) -> None:
    bottom = None
    top = None
    ...
    adb.sleep_ms(500)
    print("Navigated: find_games -> share_wall", flush=True)
    wait_for_page_content(adb)


def load_scroll_capture_cfg(adb_config: dict) -> dict:
    return dict(
        adb_config.get("sharewall_capture")
        or adb_config.get("pc_capture")
...
def main() -> int:
    ...
    if args.skip_nav or args.resume_dir:
        ...
        navigate_to_share_wall(adb, tabs)
