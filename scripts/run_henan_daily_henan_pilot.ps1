...
$Pilot = Join-Path $PSScriptRoot "pilot_henan_daily_henan_scroll.py"
$PilotArgs = @()
if ($UntilBottom) {
...
Write-Host ""
if ($UntilBottom) {
    Write-Host "Henan Daily news-henan FULL capture (until bottom)" -ForegroundColor Cyan
    Write-Host "No shot limit — stops at bottom marker or when feed no longer scrolls." -ForegroundColor Yellow
} else {
    Write-Host "Henan Daily news-henan scroll capture" -ForegroundColor Cyan
}
Write-Host "Tip: open app -> News -> Henan (河南) on device; keep screen on." -ForegroundColor Yellow
Write-Host ""
...
$Marker = Join-Path $Root "screenshots\henan_daily\latest_henan_news_henan_output_dir.txt"
