#!/usr/bin/env python3
"""河南日报 新闻-精选: scroll down + screenshot."""
...
PREFIX = "hnrb_news_jx"
FOLDER_TAG = "新闻精选"
MARKER_NAME = "latest_henan_news_featured_output_dir.txt"
...
def load_configs() -> tuple[dict, dict, dict, dict]:
    ...
    scroll_cfg = dict(adb_cfg.get("news_featured_capture") or {})
    align_cfg = first_frame_alignment_cfg(adb_cfg)
    tabs = {
        "feed_swipe": dict(adb_cfg["feed_swipe"]),
...
def main() -> int:
    ...
    parser.add_argument("--skip-nav", action="store_true", help="Already on 新闻-精选 page")
    ...
    tabs, timing, scroll_cfg, align_cfg, adb_cfg = load_configs()
    ...
    if not app_in_foreground(adb):
        print(f"Warning: {PACKAGE} may not be in foreground. Open 新闻-精选 and retry.", flush=True)
