...
$Pilot = Join-Path $PSScriptRoot "pilot_taptap_pc_scroll.py"
$PilotArgs = @()
if ($UntilBottom) {
...
Write-Host ""
Write-Host "TapTap PC games scroll capture" -ForegroundColor Cyan
Write-Host "Tip: open TapTap -> Find Games -> PC Games before running." -ForegroundColor Yellow
Write-Host "     Add -SkipNav if already on PC Games." -ForegroundColor Yellow
if ($ResumeDir) {
...
$Marker = Join-Path $Root "screenshots\taptap_lite\latest_pc_output_dir.txt"
