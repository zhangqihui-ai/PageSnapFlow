#!/usr/bin/env python3
"""TapTap 游戏分类 (b01_t04): scroll down + screenshot until bottom."""
...
TOP_TAB_ID = "t04"
PREFIX = "b01_t04"
MARKER_NAME = "latest_category_output_dir.txt"
...
def page_has_content(adb: AdbDevice, *, top_ratio: float = 0.22, bottom_ratio: float = 0.90, min_std: float = 18.0) -> bool:
    temp = Path(tempfile.gettempdir()) / f"category_probe_{time.time_ns()}.png"
    try:
        adb.screencap(temp)
...
def navigate_to_category(adb: AdbDevice, config: dict) -> None:
    bottom = None
    top = None
    ...
    if not bottom or not top:
        raise RuntimeError(f"Could not find b01/{TOP_TAB_ID} in taptap_tabs.yaml")

    adb.tap_point(bottom["point"])
    adb.sleep_ms(300)
    adb.tap_point(top["point"])
    adb.sleep_ms(500)
    print("Navigated: find_games -> category", flush=True)
    wait_for_page_content(adb)


def load_scroll_capture_cfg(adb_config: dict) -> dict:
    return dict(
        adb_config.get("category_capture")
        or adb_config.get("tapmake_capture")
        or {}
    )
...
def resolve_output_dir(args: argparse.Namespace) -> Path:
    ...
    if args.until_bottom:
        label = "category_bottom"
    elif args.full:
        label = "category_full"
    else:
        label = f"category_{args.shots}"
...
def main() -> int:
    ...
    parser.add_argument(
        "--until-bottom",
        action="store_true",
        help="Keep scrolling until consecutive dup frames or max_scroll_swipes batch",
    )
