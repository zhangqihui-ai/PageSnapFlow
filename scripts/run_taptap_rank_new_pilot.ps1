...
$Pilot = Join-Path $PSScriptRoot "pilot_taptap_rank_new_scroll.py"
$PilotArgs = @()
if ($UntilBottom) {
...
Write-Host ""
Write-Host "TapTap ranking new releases scroll capture" -ForegroundColor Cyan
Write-Host "Tip: open TapTap -> Rankings -> New Releases before running." -ForegroundColor Yellow
Write-Host "     Add -SkipNav if already on New Releases." -ForegroundColor Yellow
...
$Marker = Join-Path $Root "screenshots\taptap_lite\latest_rank_new_output_dir.txt"
