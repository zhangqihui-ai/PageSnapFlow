#!/usr/bin/env python3
"""TapTap PC游戏 (b01_t05): scroll down + screenshot until bottom."""
...
PREFIX = "b01_t05"
FOLDER_TAG = "PC游戏"
MARKER_NAME = "latest_pc_output_dir.txt"
...
def page_has_content(adb: AdbDevice, *, top_ratio: float = 0.20, bottom_ratio: float = 0.90, min_std: float = 18.0) -> bool:
    temp = Path(tempfile.gettempdir()) / f"pc_probe_{time.time_ns()}.png"
    try:
        adb.screencap(temp)
...
def navigate_to_pc_games(adb: AdbDevice, config: dict) -> None:
    bottom = None
    top = None
    ...
    adb.sleep_ms(500)
    print("Navigated: find_games -> pc_games", flush=True)
    wait_for_page_content(adb)


def load_scroll_capture_cfg(adb_config: dict) -> dict:
    return dict(
        adb_config.get("pc_capture")
        or adb_config.get("category_capture")
