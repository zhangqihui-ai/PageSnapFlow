...
$Pilot = Join-Path $PSScriptRoot "pilot_taptap_rank_hot_scroll.py"
$PilotArgs = @()
if ($UntilBottom) {
...
Write-Host ""
Write-Host "TapTap ranking hot list scroll capture" -ForegroundColor Cyan
Write-Host "Tip: open TapTap -> Rankings -> Hot Rank before running." -ForegroundColor Yellow
Write-Host "     Add -SkipNav if already on Hot Rank." -ForegroundColor Yellow
...
$Marker = Join-Path $Root "screenshots\taptap_lite\latest_rank_hot_output_dir.txt"
