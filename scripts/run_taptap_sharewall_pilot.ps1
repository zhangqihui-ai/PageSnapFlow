...
$Pilot = Join-Path $PSScriptRoot "pilot_taptap_sharewall_scroll.py"
$PilotArgs = @()
if ($UntilBottom) {
...
Write-Host ""
Write-Host "TapTap share wall scroll capture" -ForegroundColor Cyan
Write-Host "Tip: open TapTap -> Find Games -> Share Wall before running." -ForegroundColor Yellow
Write-Host "     Add -SkipNav if already on Share Wall." -ForegroundColor Yellow
if ($ResumeDir) {
...
$Marker = Join-Path $Root "screenshots\taptap_lite\latest_sharewall_output_dir.txt"
