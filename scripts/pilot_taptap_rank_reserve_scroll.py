#!/usr/bin/env python3
"""TapTap 排行榜-预约榜 (b02_t02): scroll to top, then scroll + screenshot until bottom."""
...
PREFIX = "b02_t02"
FOLDER_TAG = "预约榜"
MARKER_NAME = "latest_rank_reserve_output_dir.txt"
...
def page_has_content(adb: AdbDevice, *, top_ratio: float = 0.20, bottom_ratio: float = 0.90, min_std: float = 18.0) -> bool:
    temp = Path(tempfile.gettempdir()) / f"rank_reserve_probe_{time.time_ns()}.png"
    try:
        adb.screencap(temp)
...
def navigate_to_reserve_rank(adb: AdbDevice, config: dict) -> None:
    bottom = None
    top = None
    ...
    adb.sleep_ms(500)
    print("Navigated: ranking -> reserve_rank", flush=True)
    wait_for_page_content(adb)


def scroll_list_to_top(adb: AdbDevice, config: dict, *, repeat: int, wait_ms: int = 120) -> None:
    ...
    print(f"Scrolled list to top ({repeat} swipe(s)).", flush=True)


def load_scroll_capture_cfg(adb_config: dict) -> dict:
    return dict(adb_config.get("rank_reserve_capture") or adb_config.get("ranking_list_capture") or {})
...
def main() -> int:
    parser = argparse.ArgumentParser(description=f"TapTap ranking {PAGE_LABEL} scroll capture.")
    ...
    swipe_batch = int(scroll_cfg.get("max_scroll_swipes", 25))
    if args.until_bottom:
        dup_threshold = float(capture_overrides.get("duplicate_threshold", 0.98))
        skip_dupes = True
        ...
        max_swipes = int(scroll_cfg.get("max_scroll_swipes", tabs.get("scroll_swipes", 40)))
    else:
        dup_threshold = None
        skip_dupes = False
    ...
    if args.skip_nav or args.resume_dir:
        ...
        navigate_to_reserve_rank(adb, tabs)

    if args.until_bottom:
        mode = (
            f"until bottom (scroll {start_scroll_index}..{max_swipes}, "
            f"~8 games/page, stop at 「{scroll_cfg.get('scroll_bottom_markers', ['暂无更多'])[0]}」 "
    ...
    if not resume:
        top_repeat = int(scroll_cfg.get("scroll_to_top_repeat", 18))
        scroll_list_to_top(adb, tabs, repeat=top_repeat, wait_ms=120)
        if scroll_cfg.get("scroll_loading_markers"):
