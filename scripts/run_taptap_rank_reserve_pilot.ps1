...
$Pilot = Join-Path $PSScriptRoot "pilot_taptap_rank_reserve_scroll.py"
$PilotArgs = @()
if ($UntilBottom) {
...
Write-Host ""
Write-Host "TapTap ranking reserve list scroll capture" -ForegroundColor Cyan
Write-Host "Tip: open TapTap -> Rankings -> Reserve Rank before running." -ForegroundColor Yellow
Write-Host "     Add -SkipNav if already on Reserve Rank." -ForegroundColor Yellow
...
$Marker = Join-Path $Root "screenshots\taptap_lite\latest_rank_reserve_output_dir.txt"
...
& $Python.Source (Join-Path $PSScriptRoot "collect_screenshots.py") `
    ...
    --output $Out `
    --app taptap_lite `
    --flow scroll_rank_reserve_pilot
